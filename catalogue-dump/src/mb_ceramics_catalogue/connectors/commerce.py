"""Version-one neutral commerce snapshots emitted by platform connectors."""
...
class ContractModel(BaseModel):
    """Common behavior for replay-safe connector contracts."""
...
class StockState(ContractModel):
    ...
    @classmethod
    def _quantity_is_not_boolean(cls, value: int | None) -> int | None:
        if isinstance(value, bool):
            raise ValueError("stock quantity must be an integer, not a boolean")
        return value

    @model_validator(mode="after")
    def _quantity_and_kind_agree(self) -> StockState:
...
class CommerceOffer(ContractModel):
    ...
    @model_validator(mode="after")
    def _validity_and_availability_are_evidenced(self) -> CommerceOffer:
        if (
...
class CommerceProductSnapshot(ContractModel):
    """A platform's published view of one product at one observation time."""
