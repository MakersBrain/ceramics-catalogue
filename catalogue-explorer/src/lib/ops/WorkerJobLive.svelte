...
<div class="py-2">
	<div class="flex items-center justify-between gap-2 text-xs">
		<a class="link min-w-0 truncate font-medium" href="/ops/runs/{job.run_id}/jobs/{job.job_id}">
			{job.source}
		</a>
	...
	{#if progress}
		<dl class="mt-2 grid grid-cols-3 gap-x-3 gap-y-2 text-xs">
			<div><dt class="text-base-content/50">indexed</dt><dd class="tabular-nums font-medium">{count(progress.records)}</dd></div>
			<div><dt class="text-base-content/50">discovered</dt><dd class="tabular-nums font-medium">{count(progress.discovered)}</dd></div>
			<div><dt class="text-base-content/50">errors</dt><dd class="tabular-nums font-medium {progress.errors ? 'text-error' : ''}">{count(progress.errors)}</dd></div>
			<div><dt class="text-base-content/50">requests</dt><dd class="tabular-nums">{count(progress.requests)}</dd></div>
			<div><dt class="text-base-content/50">index rate</dt><dd class="tabular-nums">{rate ? `${rate.records.toFixed(1)}/s` : '—'}</dd></div>
			<div><dt class="text-base-content/50">request rate</dt><dd class="tabular-nums">{rate ? `${rate.requests.toFixed(1)}/s` : '—'}</dd></div>
		</dl>
