...
{:else}
	<div class="mb-5 flex flex-wrap items-start justify-between gap-3">
		<div>
			...
			<p class="text-muted-foreground text-sm">Decodo Residential · 3 GB provider ceiling · 2.4 GB operational ceiling</p>
		</div>
	...
	<section class="grid gap-4 lg:grid-cols-3">
		<div class="card bg-card shadow-sm lg:col-span-2">
			<div class="card-body p-4">
				<div class="flex items-center justify-between gap-3">
					...
					<span class="text-muted-foreground text-xs">reconciled {relative(cycle?.reconciled_at)}</span>
				</div>
				{#if cycle}
					<div class="mt-2 h-5 overflow-hidden rounded bg-muted" title={`${bytes(accounted)} accounted`}>
					...
					<div class="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
						<div><span class="text-muted-foreground block text-xs">Provider</span>{bytes(cycle.provider_reported_bytes)}</div>
						<div><span class="text-muted-foreground block text-xs">Application</span>{bytes(cycle.application_bytes)}</div>
						<div><span class="text-muted-foreground block text-xs">Reserved</span>{bytes(cycle.active_reserved_bytes)}</div>
						<div><span class="text-muted-foreground block text-xs">Headroom</span>{bytes(cycle.remaining_operational_bytes)}</div>
					</div>
					<p class="text-muted-foreground mt-2 text-xs">Today {bytes(cycle.daily_used_bytes)} · dynamic daily allowance {bytes(cycle.dynamic_daily_bytes)}</p>
					<p class="mt-3 text-xs {discrepancy > 10_000_000 ? 'text-warning' : 'text-muted-foreground'}">Provider − application discrepancy: {bytes(discrepancy)}. Provider totals remain authoritative for headroom.</p>
		...
		<div class="card bg-card shadow-sm">
			<div class="card-body gap-2 p-4">
				...
				{:else}<p class="text-muted-foreground text-sm">Viewer access is read-only.</p>{/if}
			</div>
		</div>
	</section>

	<section class="mt-5 card bg-card shadow-sm">
		<div class="card-body p-4">
	...
	<section class="mt-5 grid gap-4 xl:grid-cols-2">
		<div class="card bg-card shadow-sm"><div class="card-body p-4">
			...
			{#if admin}
				<form method="POST" action="?/createProfile" class="grid grid-cols-2 gap-2 rounded bg-muted p-3 text-sm">
			...
			{/if}
			<div class="divide-border divide-y">{#each data.profiles as profile (profile.id)}
				<div class="py-3 text-sm"><div class="flex items-center justify-between"><strong>{profile.display_name}</strong><span class="badge {profile.enabled ? 'badge-success' : 'badge-ghost'}">{profile.lifecycle}</span></div>
					<p class="text-muted-foreground text-xs">{profile.logical_name} · {profile.username_mask ?? 'not installed'} · allocation {bytes(profile.allocated_bytes)} · limit {bytes(profile.provider_traffic_limit_bytes)} · generation {profile.secret_generation}</p>
		...
		<div class="card bg-card shadow-sm"><div class="card-body p-4">
			...
			<p class="text-muted-foreground text-xs">Saving a route spends nothing. A probe streams at most 1 MB of application data and reserves a 1.1 MB provider envelope. A new session does not guarantee a unique exit.</p>
			{#if admin && profiles.length}
				<form method="POST" action="?/createRoute" class="grid grid-cols-2 gap-2 rounded bg-muted p-3 text-sm">
